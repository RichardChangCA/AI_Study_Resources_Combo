...
32. Medium, Eager Execution in Tensorflow 2.0, https://medium.com/@arun_raja/eager-execution-in-tensorflow-2-0-ad82dbd5b8b1 ,why eager execution? Automatic differentiation, quick iteration without building graphs, deep inspection of running models, dynamic models with complex control flow. Advantages of Lazy execution(1.Able to distribute operations across various compute resources for efficient and fast execution. 2. TF’s Accelerated Linear Algebra compiler can combine adjacent operations for faster execution. 3.Dataflow graphs are language independent. Thus, a saved model from Python code can be restored in another language). Advantages of Eager execution(1. More intuitive use-able to structure code similar to Python 2.Debugging is made easier where standard debugging tools can be used 3.Use of natural control flow, opposed to graph control flow, which makes dynamic models easier). TF2 has introduced the tf.function API which marries the best of both lazy and eager execution.
